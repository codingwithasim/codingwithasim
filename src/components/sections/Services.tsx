          ...
          {services.map(({ icon: Icon, ...service }, index) => {
            ...
            return (
              <ScrollFadeIn key={service.title} delay={index * 0.15}>
                <article className="relative overflow-hidden rounded-[2.75rem] border border-foreground/10 bg-white/95 p-8 md:p-12 dark:bg-foreground/[0.06]">
                  <div className="relative grid gap-10 lg:grid-cols-[1.35fr_1fr] lg:items-start">
                    <div className="space-y-8">
                      <div className="flex flex-wrap items-center gap-4 text-[0.7rem] uppercase tracking-[0.35em] text-foreground/55">
                        ...
                        <span className="inline-flex items-center rounded-full border border-foreground/15 bg-foreground/5 px-5 py-2 text-[0.65rem] font-medium tracking-[0.35em] text-foreground/70">
                          {service.label}
                        </span>
                      </div>
                      ...
                      <div className="space-y-6">
                        <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                          <div className="grid h-14 w-14 place-items-center rounded-2xl border border-foreground/10 bg-foreground/5">
                      ...
                      <div>
                        ...
                        <div className="mt-4 grid gap-3 sm:grid-cols-2">
                          {service.features.map((feature) => (
                            <div
                              key={feature}
                              className="relative overflow-hidden rounded-2xl border border-foreground/10 bg-foreground/5 px-4 py-4 text-sm font-medium text-foreground/75"
                            >
                    ...
                    <div className="space-y-6">
                      <div className="relative overflow-hidden rounded-3xl border border-foreground/10 bg-foreground/5 px-6 py-8">
                        ...
                        <div className="mt-5 grid gap-4">
                          {service.stats.map((stat) => (
                            <div key={stat.label} className="flex items-center justify-between text-sm text-foreground/70">
                              ...
                              <span className="font-semibold text-foreground/85">{stat.value}</span>
                            </div>
                          ))}
                        </div>
                      </div>
